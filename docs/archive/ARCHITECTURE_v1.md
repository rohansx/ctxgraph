# ctxgraph — Architecture (v1, ARCHIVED)

> ⚠️ **DEPRECATED — DO NOT USE THIS DOCUMENT AS A REFERENCE.**
> This is the pre-research aspirational architecture (2026-04-07). It describes crates that don't exist (`ctxgraph-ingest`, `ctxgraph-privacy`), entity types that don't match the code (`File`, `Function`, `Error`, `Package` vs. actual `Person`, `Component`, `Service`, …), and lists `gemma2:9b` as a Tier-2 default that was replaced.
>
> **Current authoritative architecture: `../ARCHITECTURE.md`**
> **Why this is preserved**: shows the original product vision before the cross-domain benchmark + deep-research synthesis forced concrete swaps. Useful as design-history record only.
